...
async function fetchCommunityByName(
    ...
    // Fetch weekly visitors count (Reddit-style metric)
    const { data: weeklyVisitors } = await supabase
        .rpc('get_weekly_visitors', { community_uuid: communityData.id })
        .abortSignal(signal);

    // Fetch weekly contributions count (posts + comments in last 7 days)
    const { data: weeklyContributions } = await supabase
        .rpc('get_weekly_contributions', { community_uuid: communityData.id })
        .abortSignal(signal);

    // Attach weekly stats to community data
    communityData.weeklyVisitors = weeklyVisitors || 0;
    communityData.weekly_visitors = weeklyVisitors || 0; // For snake_case references
    communityData.weeklyContributions = weeklyContributions || 0;
    communityData.weekly_contributions = weeklyContributions || 0; // For snake_case references
