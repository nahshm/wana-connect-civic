...
export default function Index() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (user && postsData) {
        ...
        const {
          data: votesData
        } = await supabase.from('votes').select('post_id, vote_type').eq('user_id', user.id).in('post_id', postIds);
        if (votesData) {
  ...
  const handleVote = async (postId: string, voteType: 'up' | 'down') => {
    ...
    try {
      ...
      const {
        data: existingVote
      } = await supabase.from('votes').select('*').eq('user_id', user.id).eq('post_id', postId).maybeSingle();
      if (existingVote) {
