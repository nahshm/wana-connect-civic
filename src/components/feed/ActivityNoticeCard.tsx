      ...
      <CardContent className="py-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Media Preview for Activity */}
            {(() => {
              const url = activity.metadata.media_url || activity.metadata.image_url;
              if (!url) return null;

              const isVideo = /\.(mp4|webm|ogg)$/i.test(url);

              return (
                <div className="mt-3 relative rounded-md overflow-hidden border border-border h-48 w-full max-w-md bg-black/5 group">
                  {/* Blurred Background */}
                  ...
                  <div className="absolute inset-0 z-10 flex items-center justify-center p-1">
                    {isVideo ? (
                      <video 
                        src={url} 
                        className="h-full w-auto max-w-full object-contain drop-shadow-sm rounded-sm"
                        controls={false}
                        muted
                      />
                    ) : (
